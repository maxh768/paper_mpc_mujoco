...
# second control input is not used for pendubot case
lin_u = np.array([0]) # target control = 0, no second controller
...
Q = np.eye(4)
Q[0,0] = 10 # weight lower angle to be more important
R = np.eye(1)*0.2
...
video_writer = cv2.VideoWriter('doubpend-pendubot.mp4', fourcc, 30.0, (width, height))
...
pl_ts(th1arr, th2arr, th1darr, th2darr, u1arr, u2arr, tarr, name='doubpend-pendubot.pdf')
